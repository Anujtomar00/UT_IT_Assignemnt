...
class UserValidatorTest extends org.scalatest.flatspec.AnyFlatSpec {
  ...
  "User" should "be a valid User because Company exist in DB and email is valid" in{
    ...
    when(mockedCompanyReadDto.getCompanyByName(anujEmployee.companyName)) thenReturn (Option(knoldusCompany))
    when(mockedEmail.emailIdIsValid(anujEmployee.emailId)) thenReturn(true)

    val uservalidator = new UserValidator(mockedCompanyReadDto, mockedEmail)
    val result = uservalidator.userIsValid(anujEmployee)
    assert(result)
  ...
  "User" should "be a invalid because email is not valid" in{
    ...
    when(mockedCompanyReadDto.getCompanyByName(anujEmployee.companyName)) thenReturn (Option(knoldusCompany))
    when(mockedEmail.emailIdIsValid(anujEmployee.emailId)) thenReturn(false)
  ...
  "User" should "be a invalid because company does not exist in the DB " in{
    ...
    when(mockedCompanyReadDto.getCompanyByName(anujEmployee.companyName)) thenReturn(None)
    when(mockedEmail.emailIdIsValid(anujEmployee.emailId)) thenReturn(true)
